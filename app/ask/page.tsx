...
const modes: { id: TeacherMode; label: string }[] = [
  { id: "explain", label: "Explain deeply" }, { id: "context", label: "Context" },
...
export default async function AskPage({ searchParams }: Props) {
  ...
  return (
    <main className="action-page teacher-page">
      ...
      <p className="study-lead">A full study path: understand the text, examine its context, explore original-language words, trace Scripture connections, reflect on what it reveals about God, and turn truth into obedience.</p>
      ...
      <section className="teacher-dossier">
        ...
        <div id="original" className="teacher-card"><p className="eyebrow">ORIGINAL LANGUAGE</p><h2>Greek / Hebrew word study</h2><div className="word-study-list">{insight.words.map((word) => <article key={word.word}><div><strong>{word.word}</strong><span>{word.original} · {word.transliteration}</span></div><p><b>Meaning:</b> {word.meaning}</p><p><b>Why it matters:</b> {word.whyItMatters}</p></article>)}</div><p className="teacher-note">Word notes are study aids. The exact Hebrew or Greek form, grammar and sense must always be checked in context.</p></div>
        ...
        <div className="teacher-card teacher-question"><p className="eyebrow">ASK THE TEACHER</p><h2>Take the next question</h2><p>Ask a specific question about this passage. The real AI Teacher will be connected to the same Scripture-first study framework.</p><div className="teacher-input-row"><input placeholder={`What do you want to know about ${reference}?`} aria-label={`Ask a question about ${reference}`} /><button type="button" className="primary-button button-reset">Ask</button></div></div>
      </section>
